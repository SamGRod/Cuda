...
int main()
{
	...
	{
	        // Interweaving the streams means that we now instead of doing 1 stream at a time in complete chunks, we are doing one part of each stream at a time. 
	        // To Interweave, it is in the best interest to make the ENTIRE_DATA_SET divisable by 3, so that we don't need the slow if statements in the prior code.
		//******************************************
